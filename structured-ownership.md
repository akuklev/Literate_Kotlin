...
```kotlin
init class fun KtDatabase(schemaSource : String) : GeneratedFinalClass<DbBase>
```

Type providers are meant to be used to connect standalone external resources as static objects like this:
```kotlin
restricted object Db : KtDatabase["kqdb:ourApp.prototypeDb"] {
  this(endpoint = Config.dbEndpoint)
  // other initialization if neccessary
}
```
...
does. `KtDatabase(schemaSource : String)` will be executed in compile time, and generate
...
of the late initialization (happens on the first access). If it has mandatory arguments,
it should be called in the first line of the body block immediatelly following the object
declaration.
...
Since the class generated by KtDatabase is final, its body can only contain
the `this()` call and init blocks, no new members can be introduced.
